...
本文档包含主 skill 说明中未涵盖的高级 PDF 处理功能、详细示例和额外的库。

## pypdfium2 库（Apache/BSD 许可证）

### 概述
pypdfium2 是 PDFium（Chromium 的 PDF 库）的 Python 绑定。它非常适合快速 PDF 渲染、图片生成，可作为 PyMuPDF 的替代方案。
...
#### 带 Bounding Box 坐标提取文本
```bash
# 带 Bounding Box 坐标提取文本（对结构化数据至关重要）
...
def extract_figures(pdf_path, output_dir):
    ...
    for page_num, page in enumerate(pdf):
        ...
        # 查找轮廓并提取 Bounding Box
        # （此处为简化示例 - 实际实现需要更复杂的检测逻辑）
